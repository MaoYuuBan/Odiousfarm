...
public class Ant extends Thread {
    ...
    public Ant(JLabel g,ArrayList<Food> F,ArrayList<Ant> A,ArrayList<MyImageIcon> M,Silo S,ArrayList<MySoundEffect> s){
        gamepane = g;
        FOOD = F;
        ANT = A;
        Image = M;
    ...
    public void drop(){
        dropLate--;
        if(dropLate==0){
            new Sugar(gamepane,CurX+50,CurY+50,Image.get(13),Silo,Sound);
            Sound.get(4).playOnce();
            dropLate = new Random().nextInt(2000)+2000;
        }
    }
