...
public class Cockroach extends Thread {
    //status
    private int Speed = 4;
    private int MaxHungry = 8000;
    private int Hungry = 3000;
    private int dropLate = 6000;
    ...
    private ArrayList<Cockroach> COCKROACH;
    
    public Cockroach(JLabel g,ArrayList<Food> F,ArrayList<Cockroach> A,ArrayList<MyImageIcon> M,Silo S,ArrayList<MySoundEffect> s){
        gamepane = g;
        FOOD = F;
        COCKROACH = A;
        Image = M;
    ...
    public void drop(){
        ...
        if(dropLate==0){
            ...
            new Egg(gamepane,CurX+50,CurY+50,Image.get(13),Silo,Sound);
            dropLate = new Random().nextInt(2000)+4000;
        }
    }
    ...
    public void death(){
        ...
        try { Cockroach.sleep(1000); } 
        catch (InterruptedException e) { e.printStackTrace(); }
        gamepane.remove(This_Label);
        COCKROACH.remove(this);
        gamepane.validate();
    }   
}
